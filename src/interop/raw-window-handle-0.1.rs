...
impl From<NonNullHWND>  for windows::WindowsHandle  { fn from(hwnd: NonNullHWND ) -> Self { let mut h = windows::WindowsHandle::empty(); h.hwnd = hwnd.to_ptr(); h } }
impl From<HWND>         for windows::WindowsHandle  { fn from(hwnd: HWND        ) -> Self { let mut h = windows::WindowsHandle::empty(); h.hwnd = hwnd.to_ptr(); h } }
impl From<NonNullHWND>  for RawWindowHandle         { fn from(hwnd: NonNullHWND ) -> Self { let mut h = windows::WindowsHandle::empty(); h.hwnd = hwnd.to_ptr(); RawWindowHandle::Windows(h) } }
impl From<HWND>         for RawWindowHandle         { fn from(hwnd: HWND        ) -> Self { let mut h = windows::WindowsHandle::empty(); h.hwnd = hwnd.to_ptr(); RawWindowHandle::Windows(h) } }
